...
printDiff() {
	...
	printf "%s\n" "=> => => => diff output start => => => =>"
	diff <("${compiledFile}" < "${inputFile}") "${inputFile%in.txt}out.txt"
	printf "%s\n" "<= <= <= <= diff output end <= <= <= <= <="
	printf "\n"
}
...
testIO() {
	...
	for inputFile in "${dataFolder}/"*"_in.txt"; do
		...
		diff <("${compiledFile}" < "${inputFile}") "${inputFile%in.txt}out.txt" > /dev/null
		tmp=$?
		...
		if [ $tmp -ne 0 ]; then
			...
			if ! canUseProgram "sort"; then
				...
				diff <("${compiledFile}" < "${inputFile}" | sort) <(cat "${inputFile%in.txt}out.txt" | sort) > /dev/null
				tmp=$?
			fi
